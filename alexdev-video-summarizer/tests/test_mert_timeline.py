#!/usr/bin/env python3
"""
Isolated MERT Timeline Service Test

Tests the MERT music understanding timeline service independently.
...
Output:
- build/{video_name}/audio_timelines/mert_music_timeline.json
"""
...
from services.mert_timeline_service import MERTTimelineService
...
def test_mert_timeline_service():
    """Test MERT timeline service with existing no_vocals.wav files"""
    print("=" * 70)
    print("Isolated MERT Timeline Service Test")
    print("=" * 70)
    ...
    try:
        ...
        # Initialize MERT timeline service
        print("Initializing MERTTimelineService...")
        mert_service = MERTTimelineService(config)
        ...
        for file_info in no_vocals_files:
            ...
            try:
                ...
                # Generate timeline using MERT service with all features enabled
                timeline = mert_service.generate_and_save(
                    audio_path=str(no_vocals_path),
                    source_tag="mert_music",
                    optimization={
                        "analyze_music_features": True,
                        "analyze_genre_classification": True,
                        "analyze_mood_classification": True,
                        "analyze_instruments_classification": True,
                        "analyze_style_classification": True
                    }
                )
                ...
                # Results summary
                output_file = file_info['audio_timelines_dir'] / "mert_music_timeline.json"
                
                results.append({
                    "video_name": video_name,
                    "success": True,
                    "events_count": len(timeline.events),
                    "spans_count": len(timeline.spans),
                    "processing_time": processing_time,
                    "output_file": output_file
                })
                
                print(f"  [SUCCESS] {len(timeline.events)} music events, {len(timeline.spans)} spans in {processing_time:.2f}s")
        ...
        # Cleanup service
        mert_service.cleanup()
        
        # Summary
        print("=" * 70)
        print("MERT Timeline Test Results:")
        print("=" * 70)
        ...
        if successful:
            ...
            for result in successful:
                print(f"  • {result['video_name']}: {result['events_count']} events, {result['spans_count']} spans ({result['processing_time']:.2f}s)")
        ...
        print()
        print("[INFO] Test includes: Genre, Mood, Style, Instruments, Music Features")
        print("[INFO] Timeline JSON files ready for integration with main pipeline")
        
        return len(failed) == 0
        
    except Exception as e:
        print(f"[ERROR] MERT timeline test failed: {e}")
        return False

def main():
    """Run isolated MERT timeline test"""
    print("Isolated ML Timeline Service Tests")
    print("==================================")
    print("Testing: MERT Music Understanding Timeline Service")
    print()
    
    success = test_mert_timeline_service()
    
    if success:
        print("\n[SUCCESS] MERT timeline test completed successfully!")
